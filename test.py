...
TOKEN = "8801882317:AAFVcLH8yo9w7CY8eFGgYLK2rLY8fC0Z1tw"
ADMIN_ID = 6968399046
...
def handle_text(message):
    ...
    if is_admin(user_id):
        ...
        if state:
            ...
            if step == "delete_code":
                if text.isdigit():
                    result = movies.delete_many({"$or": [{"code": text}, {"code": int(text)}]})
                else:
                    result = movies.delete_many({"code": text})
